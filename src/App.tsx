...
import { bootstrapSpeculativePipeline, setExecutionMode } from "@/rag/pipeline";
...
export default function App() {
  ...
  const [scrollMode, setScrollMode] = useState<"container" | "page">(
    "container",
  );
  // ADD THESE STATES:
    ...
    const bootSequence = async () => {
      ...
      bootstrapSpeculativePipeline(targetModel!, draftModel, (text) => {
        ...
        .catch((err) => {
          ...
          const isCacheFailure = msg.includes("Failed to execute 'add' on 'Cache'") || msg.includes("Request failed") || msg.includes("shard fetch failed");
          const isF16Target = targetModel?.includes('q4f16') && !targetModel?.includes('abliterated');

          if (isCacheFailure && isF16Target) {
            console.warn('[App] f16 cache failure — falling back to q4f32 automatically.');
            bootLockRef.current = false;
            setIsBooting(false);
            setTargetModel('Llama-3.2-1B-Instruct-q4f32_1-MLC');
          } else {
            setIsBooting(false);
            setBootError(`Initialization Failed: ${msg}`);
            bootLockRef.current = false;
          }
        });
    };
      ...
      lenis = new Lenis({
        ...
        content:
          container === window
            ...
            : container && "firstElementChild" in container
              ? ((container as HTMLElement).firstElementChild as HTMLElement)
              : undefined,
        duration: 1.2,
    ...
    if (scrollMode === "page") {
      ...
      if (viewport) {
        ...
        observer = new MutationObserver(() => {
          const newViewport = document.querySelector(
            "[data-radix-scroll-area-viewport]",
          );
          if (newViewport) {
  ...
  const runtime = useLocalRuntime({
    run: async function* ({ messages, abortSignal }) {
      try {
        ...
        setActiveProgressCallback((msg) => {
          queue.push(msg);
        });

        ragApp
          .invoke({ query: queryText }, { signal: abortSignal })
          .then((state) => {
            queue.push({ type: "done", text: state.answer });
            done = true;
          })
          .catch((e) => {
            error = e;
        ...
        while (!done || queue.length > 0) {
          if (queue.length > 0) {
            ...
            if (msg.type === "done") {
              ...
              setTps(
                Math.round(
                  (tokenCount / ((performance.now() - startTime) / 1000)) * 10,
                ) / 10,
              );
              setContextFill(
                Math.min(100, (displayedText.length / 8192) * 100),
              );
              yield {
                content: [
                  {
        ...
        if (error) {
          yield {
            content: [
              { type: "text", text: `System error: ${error.message || error}` },
            ],
          };
        }
      } catch (err: any) {
        yield {
          content: [
            { type: "text", text: `System error: ${err.message || err}` },
          ],
        };
      }
    },
  ...
  return (
    <TooltipProvider>
      <AssistantRuntimeProvider runtime={runtime}>
        <div
          className={`relative flex w-full bg-zinc-950 text-white selection:bg-violet-500/30 ${scrollMode === "container" ? "h-[100dvh] overflow-hidden" : "min-h-[100dvh]"}`}
        >
          <DocumentDropzone
            onProgress={(status) => {
          ...
          {/* ADD THIS BLOCK: Hardware Telemetry HUD */}
          {engineReady && (
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  ...
                  bootLockRef.current = false;
                  setExecMode(newMode);
                  import("@/rag/pipeline").then((m) =>
                    m.setExecutionMode(newMode),
                  );
                }}
            ...
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-10 pointer-events-none backdrop-blur-md bg-black/40 transition-opacity duration-1000">
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                <div className="flex items-center gap-3 text-white/80 text-xs font-mono tracking-widest uppercase">
                  <div
                    className={`w-2 h-2 rounded-full ${bootError ? "bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]" : "bg-violet-400 shadow-[0_0_10px_rgba(167,139,250,0.8)]"} animate-pulse`}
                  />
                ...
                <div className="mt-8 flex flex-col gap-3">
                  <div
                    className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-lg border border-white/10 pointer-events-auto cursor-pointer"
                    onClick={() => !engineReady && setUseZeroCopy(!useZeroCopy)}
                  >
                    <div
                      className={`w-10 h-5 rounded-full transition-colors relative ${useZeroCopy ? "bg-violet-500" : "bg-white/20"}`}
                    >
                      <div
                        className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${useZeroCopy ? "left-[22px]" : "left-[2px]"}`}
                      />
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] text-white/50">
                        {useZeroCopy
                          ? "Bypasses iOS Cache Limits (Slower Boot)"
                          : "Uses IndexedDB Cache (Fast Boot)"}
                      </span>
                    </div>
                  </div>
                  <button
          ...
          {engineReady && globalStatus && (
            <div className="absolute top-6 left-1/2 -translate-x-1/2 z-40 pointer-events-none">
              <div className="glass-panel px-5 py-2.5 rounded-full border border-violet-500/40 flex items-center gap-3 shadow-[0_0_20px_rgba(139,92,246,0.2)] backdrop-blur-xl">
                ...
                <span className="text-xs font-mono text-violet-100 tracking-wider">
                  {globalStatus}
                </span>
              </div>
            </div>
          ...
          >
            {engineReady && (
              <div className="absolute top-4 right-4 z-50 flex gap-2 items-center pointer-events-auto">
                ...
                <div className="flex items-center gap-1 bg-black/40 backdrop-blur-md rounded-full px-2 py-1 border border-white/10">
                  <span className="text-[10px] text-white/50 uppercase tracking-widest px-2">
                    BG
                  </span>
            ...
            <SpatialPanel
              depth={20}
              className={`w-full h-full rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-700 ${getBorderClass(borderStyle)}`}
            >
              <PanelGroup direction="horizontal" className="w-full h-full">
                ...
                >
                  <Suspense
                    fallback={
                      <div className="flex h-full items-center justify-center text-violet-400/50 animate-pulse font-mono text-sm">
                        Mounting Secure Boundary...
                      </div>
                    }
                  >
          ...
          {engineReady && (
            <div className="engine-ready-indicator absolute opacity-0 w-0 h-0 pointer-events-none" />
          )}
        </div>
